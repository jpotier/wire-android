...
import com.waz.zclient.glide.{GlideDrawable, ObservableImageTarget, WireGlide}
  ...
  private lazy val previewImage = returning(view[ImageView](R.id.giphy_preview)) { vh =>
    ...
    gifImage.onUi {
      case Some(uri) => vh.foreach { v =>
        GlideDrawable(uri).into(ObservableImageTarget(v)).resourceState.map {
          case ObservableImageTarget.ResourceState.Ready => true
          case _ => false
        }.pipeTo(isGifShowing)
      }
